...
// monetary amount.
const (
	AmountMegaDeveus  AmountUnit = 6
	AmountKiloDeveus  AmountUnit = 3
	AmountDeveus      AmountUnit = 0
	AmountMilliDeveus AmountUnit = -3
	AmountMicroDeveus AmountUnit = -6
	AmountGosh        AmountUnit = -8
)

// String returns the unit as a string.  For recognized units, the SI
// prefix is used, or "Gosh" for the base unit.  For all unrecognized
// units, "1eN Deveus" is returned, where N is the AmountUnit.
func (u AmountUnit) String() string {
	switch u {
	case AmountMegaDeveus:
		return "MDeveus"
	case AmountKiloDeveus:
		return "kDeveus"
	case AmountDeveus:
		return "Deveus"
	case AmountMilliDeveus:
		return "mDeveus"
	case AmountMicroDeveus:
		return "μDeveus"
	case AmountGosh:
		return "Gosh"
	default:
		return "1e" + strconv.FormatInt(int64(u), 10) + " Deveus"
	}
}
...
//
// NewAmount is for specifically for converting Deveus to Gosh.
...
// ToDeveus is the equivalent of calling ToUnit with AmountDeveus.
func (a Amount) ToDeveus() float64 {
	return a.ToUnit(AmountDeveus)
}
...
// String is the equivalent of calling Format with AmountDeveus.
func (a Amount) String() string {
	return a.Format(AmountDeveus)
}
